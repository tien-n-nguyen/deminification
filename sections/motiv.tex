...
\code{preSelectionRange.setEnd} at line 9.
...
name \code{emptyBlocksIndex} for the variable \code{y} at line 24 in
Figure~\ref{example_sim}: \code{if (y != -1)}. From that comparison, a
model could learn that \code{y} is of the type \code{int}. Now,
knowing that it is an integer, a model could combine that with the
knowledge learned from line 23 (\code{var y =
  this.getIndexRelativeToAdjacentEmptyBlocks(...)}), and predict the
name for \code{y} could be something relevant to {\em ``Index''}.
%at line 24 of Figure~\ref{example_sim}, \code{if (y != -1)}, a model
%can derive that \code{y} is of the type of \code{int}. Together with
%the line 23, \code{var y =
...
\code{trailingImageCount}, then its type is likely to be \code{int}.
