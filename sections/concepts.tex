...
\code{r.cloneRange()}, \code{r.startOffset}, \code{r.startContainer}.
...
in such an assignment or an argument. For example, \code{range\_} and
\code{getRangeAt} are in accordance with each other in \code{range\_ =
...
Regarding type inference, Figure~\ref{python-types} shows Python's
...
derived by the function call \code{len}. The subscript is used to to
